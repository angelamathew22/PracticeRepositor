...
--couting authors per each state minium 2 authors from each state
...
select state,COUNT(*) as 'Author Count' from authors group by state
select state,COUNT(*) as 'Author Count' from authors where contract=1 group by state
select state,COUNT(*) as 'Author Count' from authors where contract=1 group by state order by [Author Count]
